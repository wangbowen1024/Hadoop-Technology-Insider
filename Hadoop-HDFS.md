...
简单来说，RPC 就是允许程序调用其他机器上进程的过程（也可以是同一台机器不同进程）。即机器A上的进程调用机器B上的进程时，A上调用的进程被挂起，B上的被调用的进程开始执行。调用方将参数信息传递给被调用方，然后通过传回的结果得到信息。在这个过程中A是客户端，B是服务器。同时，编程人员看不到任何的信息传递，就像是调用本地方法一样。下图是 RPC 调用示例图：

<img src="https://gitee.com/wangbowen97/pic-go/raw/master/image-20211203005702175.png" alt="image-20211203005702175" style="zoom: 50%;" />

> **解析**
>
> 当 theClient 需要调用一个远程过程的时候，通过 theClient 将参数打包成一个消息，并附加被调用过程的名字（说明要调用服务器上的哪一个过程），然后发送消息到服务器。theClient 发送完消息后，必须等待服务器的应答，这个时候，执行流是空闲的。
>
> RPC 的 Server 运行时会阻塞在接受消息的调用上，当接收到客户端的请求后，它会进行解包以获得请求参数。在调用结束以后，返回值通过主程序打包并返回客户端，通知客户端调用结束。



RPC 的实现一般采用如下图所示的形式：

<img src="https://gitee.com/wangbowen97/pic-go/raw/master/image-20211203011215339.png" alt="image-20211203011215339" style="zoom:50%;" />

##### Java 远程方法调用（RMI）

Java 远程方法调用（RMI）是 Java 的一个核心 API 和类库，允许一个 Java 虚拟机上运行的 Java 程序调用不同虚拟机上运行的对象中的方法。在某种程度上，RMI 可以看成是 RPC 的 Java 升级版。**同时 RMI 依赖于 Java 序列化机制，它将调用的参数和返回值（java基本类型或者对象）序列化并在网络中传输。**

和 RPC 一样，包含 RMI 的 Java 应用程序通常包括服务器程序和客户端程序。开发步骤一般如下：

1. 编写远程接口
   * 远程接口必须要声明为 public
   * 远程接口必须继承 java.rmi.Remote
   * 接口方法必须声明 throws RemoteException
2. 编写远程接口实现类，并实现远程接口方法
   * 远程接口实现类必须继承UnicastRemoteObject，并实现远程接口
3. 编写服务器端程序
   * 创建远程接口实例
   * 创建端口
   * 绑定远程对象到名字（URL）
...
具体实现 Demo，参见 GITHUB：`package org.bigdata.hadoop.common.rmi;`
